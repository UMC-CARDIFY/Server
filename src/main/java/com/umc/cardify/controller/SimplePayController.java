...
public class SimplePayController {
  ...
  public ResponseEntity<BillingKeyResponse.RequestBillingKeyRes> requestBillingKey(
    ...
    try {
      BillingKeyResponse.RequestBillingKeyRes response = simplePayService.requestBillingKey(request);
  ...
  public ResponseEntity<?> approveBillingKey(@RequestBody BillingKeyRequestDTO.ApproveBillingKeyReq request) {
    ...
    try {
      ...
      BillingKeyResponse.ApproveBillingKeyRes response =
          simplePayService.approveBillingKey(request);
  ...
  public ResponseEntity<?> getBillingStatus(@PathVariable String merchantUid) {
    try {
      BillingKeyResponse.BillingStatusRes status = simplePayService.getBillingStatus(merchantUid);
  ...
  public ResponseEntity<Void> cancelPayment(
    ...
    try {
      boolean success = simplePayService.cancelPayment(request);
      return success
  ...
  @Operation(summary = "빌링키 상태 상세 조회", description = "포트원 결제 고유 ID로 결제 정보와 빌링키 요청 정보를 조회합니다.")
  public ResponseEntity<BillingKeyResponse.CheckBillingKeyStatusRes> checkBillingKeyStatus(
      @Parameter(description = "포트원 결제 고유 ID (imp_uid)", required = true, example = "imp_1234567890")
      @RequestParam String impUid) {

    BillingKeyResponse.CheckBillingKeyStatusRes response =
        simplePayService.checkBillingKeyStatus(impUid);
